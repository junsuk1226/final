...
public class Paging {
	...
	// 이전 기능과 다음기능을 적용할 것인지? 안할 것인지?를 결정하는 변수
	...
	public Paging(int nowPage, int totalRecord, int numPerPage, int pagePerBlock) {
		...
		// 총 게시물의 수(totalRecord)가 들어왔으니 총 페이지 수를 구하자!
		totalPage = (int) Math.ceil((double) totalRecord / numPerPage);

		// 총 페이지를 구했으니 현재 페이지가 총 페이지 값을 초과 했는지? 확인하자!
		if (nowPage > totalPage)
			nowPage = totalPage;

		// 현재 블럭의 시작페이지값과 마지막페이지 값을 구하자!
		startPage = (int) ((nowPage - 1) / pagePerBlock) * pagePerBlock + 1;
		endPage = startPage + pagePerBlock - 1;

		// 마지막페이지 값이 총 페이지 값보다 크면 안된다.
		if (endPage > totalPage)
			endPage = totalPage;

		// 이제는 한 페이지에 보여질 게시물들의 시작 행번호(begin)와 마지막 행번호(end)를 구한다.
		...
		for (int i = startPage; i <= endPage; i++) {
			// i와 nowPage가 같다면 비활성화, 다를 때만 링크를 걸어서 활성화 시킨다.
			if (i == nowPage) {
